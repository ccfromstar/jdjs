...
*/
	...
	/*模板0 奇数行模板*/			"<span>[<a href=\"../{9}\" title=\"{4}\">{3}</a>]<a href=\"../{10}\" title=\"{4}\">{1}</a></span>",
	/*模板1 奇数行模板*/			"<a href=\"../{9}\" title=\"{4}\">{1}</a>",
	...
	/*模板0 偶数行模板*/			"<span style=\"background-color: #ffffcc;\">[<a href=\"../{9}\" title=\"{4}\">{3}</a>]<a href=\"../{10}\" title=\"{4}\">{1}</a></span>",
	/*模板1 偶数行模板*/			"<a href=\"../{9}\" title=\"{4}\"><b>{1}</b></a>",
    ...
    private void OutPut(DataTable dt, string template, string alternatingTemplate)
    {
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            result += string.Format((i % 2 == 0 ? template : alternatingTemplate), dr["tid"].ToString(), (length == -1 ? title : Utils.GetUnicodeSubString(title, length, "")), forum.Fid, forum.Name, title, thumbnail, "", "", "", Urls.ShowForumAspxRewrite(forum.Fid, 0, forum.Rewritename), Urls.ShowTopicAspxRewrite(tid, 0));
            i++;
        }
